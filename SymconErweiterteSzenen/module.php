	...
	protected function setAllOnChangeEventsForHits () {
		...
		if (count($allScenes) > 0) {
			...
			foreach ($allScenes as $scene) {
				...
				$ary = (array) wddx_deserialize($scene);
	...
	public function onTargetChanged () {
		...
		if (count($allScenes) > 0) {
			...
			if ($isValidScene) {
				...
				$fittedVal = str_replace("Scene", "", $sceneOBJ['ObjectIdent']);
				$fittedVal = str_replace("Data", "", $fittedVal);
				
				$fittedVal = $fittedVal - 1;

				SetValue($szenenVariable, $fittedVal);
